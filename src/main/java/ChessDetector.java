...
public class ChessDetector {
    private static final Logger log = LoggerFactory.getLogger(ChessDetector.class);
    private static int Seed = 2901;
    private static double DetectionThreshold = 0.6; // Threshold
    private static int nBoxes = 8; // number of anchor boxes
    private static double lambdaNoObj = 0.5;
    private static double lambdaCoord = 5.0;
    private static double[][] priorBoxes = {{1,1},{2,3},{3, 4}, {3, 6}, {4, 7}, {7, 8}, {4, 9},{5,5}}; // Anchor boxes
    ...
    private static File modelFilename = new File(System.getProperty("user.dir"), "generated-models/ChessDetector_yolov2.zip");
    private static ComputationGraph model;
    private static Frame frame = null;
    private static final Scalar RED = RGB(255, .0, 0);
    private static final Scalar BLUE = RGB(0, 0, 255);
    private static Scalar[] colormap = {RED, RED,RED,RED,RED,RED,BLUE,BLUE,BLUE,BLUE,BLUE,BLUE};
    private static String labeltext = null;
    private static final String featurizeExtractionLayer = "conv2d_5";

    public static void main(String[] args) throws Exception {

        //   Create iterators
        ChessDataSetIterator.setup();
        RecordReaderDataSetIterator trainIter = ChessDataSetIterator.trainIterator(BatchSize);
        RecordReaderDataSetIterator testIter = ChessDataSetIterator.testIterator(1);
        ...
        if (modelFilename.exists()) {
            ...
            System.out.println(model.summary(InputType.convolutional(
                    ChessDataSetIterator.yoloheight,
                    ChessDataSetIterator.yolowidth,
                    nClasses)));

            // Training and Save model.
            log.info("Train model...");
            // set server and listeners
                ...
                thread = new Thread(() ->
                {
                    while (frame != null) {
                        try {
                            ...
                            resize(rawImage, resizeImage, new Size(ChessDataSetIterator.yolowidth, ChessDataSetIterator.yoloheight));
    ...
    private static Mat drawResults(List<DetectedObject> objects, Mat mat, int w, int h) {
        for (DetectedObject obj : objects) {
            ...
            int x1 = (int) Math.round(w * xy1[0] / ChessDataSetIterator.gridWidth);
            int y1 = (int) Math.round(h * xy1[1] / ChessDataSetIterator.gridHeight);
            int x2 = (int) Math.round(w * xy2[0] / ChessDataSetIterator.gridWidth);
            int y2 = (int) Math.round(h * xy2[1] / ChessDataSetIterator.gridHeight);
            //Draw bounding box
